 ...
 return [lastOctet[1],lastOctet[2]];	
  ...
  function checkIP(iPText){
    ...
    function calculateOrigin(){
			...
			// Calculate network address
			 ...
			 // Calculate Number of Subnet and Host
				...
				if(neededHosts < ipHost) {
					return "There are enough hosts to fit the number of needed hosts."
				} else {
					return "There are not enough hosts to fit the number of needed hosts."
				};
			}
		  }
		...
		document.getElementById("subnetDisplay").innerHTML = "Subnet Mask : " + ipA  +"." + ipB +"." + ipC +"." + ipD ;
		...
		document.getElementById("checkNeededHosts").innerHTML = "Needed hosts status : " + checkNeededHosts();
         dropSelect = 0;
